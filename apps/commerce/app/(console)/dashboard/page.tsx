...
import { orgApi, commerceApi, resolveOrgId } from '@/lib/api';
import { Store, Package, BarChart2, MapPin } from 'lucide-react';
import {
  BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Cell,
} from 'recharts';

type OrgSummary = { id: string; name: string; slug: string; role: string };
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const orgRes = await orgApi.listSummary();
        const orgs = orgRes.data as OrgSummary[];
        const orgId = resolveOrgId(orgs);
        const active = orgs.find((item) => item.id === orgId) ?? null;
        if (!mounted || !orgId || !active) return;
        setOrg(active);

        const [storeRes, productRes] = await Promise.all([
          commerceApi.listStores(orgId),
          commerceApi.searchProducts(orgId),
        ]);
