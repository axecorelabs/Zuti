...
const STATUS_STYLE: Record<ScanSession['status'], string> = {
  ...
  expired: 'bg-zinc-300/40 dark:bg-zinc-700/40 text-zinc-600 dark:text-zinc-400 border-zinc-400/40 dark:border-zinc-600/40',
...
export default function ScanSessionsPanel({ orgId, productId }: { orgId: string; productId: string }) {
  ...
  return (
    <div className="mt-6 border-t border-zinc-200/60 dark:border-zinc-800/60 pt-5">
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          <Link2 className="w-4 h-4 text-zinc-500 dark:text-zinc-500" />
          <h3 className="text-sm font-medium text-zinc-800 dark:text-zinc-200">Scanning links</h3>
        </div>
        <button onClick={() => setShowForm((v) => !v)} className="flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 text-zinc-800 dark:text-zinc-200 transition-colors">
          <Plus className="w-3.5 h-3.5" /> New link
        </button>
      </div>
      <p className="text-xs text-zinc-400 dark:text-zinc-600 mb-3">Temporary public links so door staff can scan tickets without an account. Event-scoped, expiring, admit-only.</p>

      {showForm && (
        <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-100/40 dark:bg-zinc-900/40 p-3 mb-3 space-y-2.5">
          <div>
            <label className="block text-[11px] text-zinc-500 dark:text-zinc-500 mb-1">Label (optional)</label>
          ...
          <div>
            <label className="block text-[11px] text-zinc-500 dark:text-zinc-500 mb-1">Expires (optional — defaults to event end + 6h)</label>
          ...
          <div className="flex justify-end gap-2">
            <button onClick={() => setShowForm(false)} className="px-3 py-1.5 rounded-lg text-xs text-zinc-600 dark:text-zinc-400 hover:text-zinc-800 dark:hover:text-zinc-200">Cancel</button>
            <button onClick={create} disabled={creating} className="px-3 py-1.5 rounded-lg bg-brand-600 hover:bg-brand-500 disabled:opacity-50 text-white text-xs font-medium">
      ...
      {loading ? (
        <div className="h-12 rounded-xl bg-zinc-100 dark:bg-zinc-900 animate-pulse" />
      ) : sessions.length === 0 ? (
        <p className="text-xs text-zinc-400 dark:text-zinc-600 py-2">No scanning links yet.</p>
      ) : (
        <div className="space-y-1.5">
          {sessions.map((s) => (
            <div key={s.id} className="flex items-center gap-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-100/40 dark:bg-zinc-900/40 px-3 py-2.5">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-sm text-zinc-800 dark:text-zinc-200 truncate">{s.label || 'Scanning link'}</span>
                ...
                <p className="text-[11px] text-zinc-400 dark:text-zinc-600 flex items-center gap-1 mt-0.5">
              ...
              {s.status === 'active' && (
                <>
                  <button onClick={() => copy(s)} title="Copy link" className="p-1.5 rounded-lg text-zinc-500 dark:text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200 hover:bg-zinc-200 dark:hover:bg-zinc-800">
                    {copiedId === s.id ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                  </button>
                  <button onClick={() => revoke(s)} title="Revoke link" className="p-1.5 rounded-lg text-zinc-500 dark:text-zinc-500 hover:text-red-400 hover:bg-zinc-200 dark:hover:bg-zinc-800"><Ban className="w-3.5 h-3.5" /></button>
                </>
              )}
            </div>
          ))}
