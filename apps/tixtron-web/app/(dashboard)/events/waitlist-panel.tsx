...
const STATUS_STYLE: Record<WaitlistEntry['status'], string> = {
  WAITING: 'bg-zinc-300/40 dark:bg-zinc-700/40 text-zinc-700 dark:text-zinc-300 border-zinc-400/40 dark:border-zinc-600/40',
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-2">
        <Hourglass className="w-3.5 h-3.5 text-zinc-500 dark:text-zinc-500" />
        <p className="text-xs font-medium text-zinc-500 dark:text-zinc-500">Waitlist · {entries.length}</p>
      </div>
      <div className="space-y-1.5">
        {entries.map((e) => {
          const position = positionOf(e);
          return (
            <div key={e.id} className="flex items-center gap-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-100/40 dark:bg-zinc-900/40 px-3 py-2.5">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-sm text-zinc-800 dark:text-zinc-200 truncate">{e.customerName || e.customerEmail}</span>
              ...
              <button onClick={() => cancel(e)} disabled={cancelling === e.id} title="Remove from waitlist" className="p-1.5 rounded-lg text-zinc-500 dark:text-zinc-500 hover:text-red-400 hover:bg-zinc-200 dark:hover:bg-zinc-800 disabled:opacity-40">
